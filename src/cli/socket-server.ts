 ...
 * frame, closes.
 *
 * Lives at data/clawie.sock (separate from data/cli.sock, which the existing
...
export async function startCliServer(socketPath: string = DEFAULT_SOCKET_PATH): Promise<void> {
  ...
  try {
    ...
    if (e.code !== 'ENOENT') {
      log.warn('Failed to unlink stale clawie socket (will try to bind anyway)', { socketPath, err });
    }
  }
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    s.listen(socketPath, () => {
      try {
        ...
        log.warn('Failed to chmod clawie socket (continuing)', { socketPath, err });
      }
      log.info('clawie CLI server listening', { socketPath });
      resolve();
    });
  });
}
...
function handleConnection(conn: net.Socket): void {
  ...
  conn.on('error', (err) => {
    log.warn('clawie CLI server connection error', { err });
  });
}
...
async function handleFrame(conn: net.Socket, line: string): Promise<void> {
  ...
  // Host caller — connecting to data/clawie.sock requires file-system access
...
function write(conn: net.Socket, frame: ResponseFrame): void {
  try {
    ...
    log.warn('Failed to write clawie CLI response', { err });
  }
}
