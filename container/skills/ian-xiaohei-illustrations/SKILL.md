...
如果用户明确要求“生成 / 输出 / 做图 / 帮我生成”，不要停下来等确认；用 `mcp__clawie__generate_image` 工具每张单独生成（每次调用只生成一张，传入 `prompt`，并设 `aspect_ratio: "16:9"`；可用 `output_path` 指定保存位置）。不要把多张图拼在一张里。生成后用 `send_file` 把图片发给用户。
