...
This document is the contract for Clawie's end-to-end scripted setup
(`bash clawie.sh` → `pnpm run setup:auto`). Read it before adding a new
...
| File | Role |
|---|---|
| `clawie.sh` | Top-level wrapper. Phase 1 (bootstrap) and phase 2 (setup:auto) orchestration. Writes bootstrap's raw log + progression entry. |
