...
**`nanocoai/clawie`** (upstream) — core engine with skill definitions (`.claude/skills/`). No channel code on `main`.

**Channel forks** (`clawie-whatsapp`, `clawie-telegram`, `clawie-slack`, etc.) — each fork = upstream + one channel's code applied. Users clone upstream, then merge a fork into their clone to add a channel.
...
user clones upstream main
  ├── merges clawie-whatsapp fork  → adds WhatsApp
...
```bash
# In your local clawie checkout
git checkout main && git pull

# For a fork:
git fetch clawie-whatsapp
git checkout -B whatsapp-merge clawie-whatsapp/main
git merge main
...
git push clawie-whatsapp HEAD:main
...
1. Fork `clawie` to `clawie-{channel}`
