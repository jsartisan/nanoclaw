# Running Clawie in Docker Sandboxes (Manual Setup)

This guide walks through setting up Clawie inside a [Docker Sandbox](https://docs.docker.com/ai/sandboxes/) from scratch — no install script, no pre-built fork. You'll clone the upstream repo, apply the necessary patches, and have agents running in full hypervisor-level isolation.
...
Enter the sandbox:
```bash
docker sandbox run shell-clawie-workspace
```
...
## Step 3: Clone and Install Clawie

Clawie must live inside the workspace directory — Docker-in-Docker can only bind-mount from the shared workspace path.
...
WORKSPACE=/Users/you/clawie-workspace

# Move into workspace so DinD mounts work
mv clawie "$WORKSPACE/clawie"
cd "$WORKSPACE/clawie"
...
Clawie needs several patches to work inside a Docker Sandbox. These handle proxy routing, CA certificates, and Docker-in-Docker mount restrictions.
...
In `src/container-runtime.ts`, the `cleanupOrphans()` function matches containers by the `clawie-` prefix. Inside a sandbox, the sandbox container itself may match (e.g., `clawie-docker-sandbox`). Filter out the current hostname:
...
```bash
cd ~ && git clone https://github.com/nanocoai/clawie.git && mv clawie /path/to/workspace/clawie
```
