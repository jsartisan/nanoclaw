...
2. **Filtering**: deciding which messages to forward to the host for processing. This can be stateless (regex trigger match) or stateful (e.g., "was the bot mentioned in this thread at some point? If so, forward all subsequent messages"). The adapter receives a stream of unfiltered platform messages and decides which ones to pass on. How it decides is an implementation detail — Clawie doesn't know or care.
...
- A bridge wraps the Chat instance + adapter to conform to Clawie's standard channel interface
- Chat SDK handles: webhook parsing, dedup, message history, platform API calls, rich content delivery
- Clawie handles: routing, agent lifecycle, session management

**Chat SDK's subscription model:**

Chat SDK has its own thread-level subscription concept (distinct from Clawie's channel-level registration):
...
This is sub-channel granularity. Clawie registers at the channel level ("listen to this Discord channel"). Chat SDK subscribes at the thread level ("track this specific Slack thread"). The bridge lets Chat SDK manage its own subscriptions internally — Clawie doesn't interfere with or replicate this.
...
Non-Chat-SDK channels (WhatsApp via Baileys, Gmail, custom integrations) implement the Clawie channel interface directly — no bridge, no Chat SDK types.
...
**`chat`** — simple Clawie format. Any channel can produce this.
```json
{
  "sender": "John",
  "senderId": "user123",
...
**`chat`** — simple Clawie format. Clawie channel delivers via `sendMessage(text)`.
```json
...
Clawie does not stream tokens to users. The Claude Agent SDK's `query()` yields complete results. The agent-runner writes one complete message to messages_out per result. The host delivers complete messages to channels.
...
Clawie is customized via skills — branches that get merged into the user's installation. Different skills add different capabilities (channels, integrations, behaviors). The code must be structured so that:
...
- Commands that don't make sense in the Clawie context or could cause issues
...
- **[api-details.md](api-details.md)** — Channel adapter interface (Clawie + Chat SDK bridge), message content examples, host delivery logic
