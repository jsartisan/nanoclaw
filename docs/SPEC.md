...
│  │    • mcp__clawie__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── clawie.log               # Host stdout
│   └── clawie.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.clawie.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'clawie-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your clawie installation
...
Clawie uses a hierarchical memory system based on CLAUDE.md files.
...
Clawie has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__clawie__schedule_task]
        {
...
The `clawie` MCP server is created dynamically per agent call with the current group's context.
...
Clawie runs as a single macOS launchd service.

### Startup Sequence

When Clawie starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Clawie containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/clawie.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/clawie.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.clawie.plist ~/Library/LaunchAgents/
...
# Check status
launchctl list | grep clawie

# View logs
tail -f logs/clawie.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep clawie` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Clawie auto-starts container runtime but may fail |
...
- `logs/clawie.log` - stdout
- `logs/clawie.error.log` - stderr
