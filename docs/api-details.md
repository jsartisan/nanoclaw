...
interface InboundMessage {
  id: string;
  kind: 'chat' | 'chat-sdk';
  content: unknown;       // JSON blob — Clawie chat format or Chat SDK SerializedMessage
  timestamp: string;
}
...
Wraps a Chat SDK adapter + Chat instance to conform to the Clawie ChannelAdapter interface. Trunk ships the bridge and the channel registry only — platform-specific Chat SDK adapters (Discord, Slack, Telegram, etc.) and native adapters (WhatsApp/Baileys) are installed by the `/add-<channel>` skills from the `channels` branch.
...
### Native Clawie Channel (no Chat SDK)
