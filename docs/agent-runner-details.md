...
1. **Agent-runner core** — owns the poll loop, message formatting, DB reads/writes, MCP tool implementations, routing, status management, media handling. This is Clawie-specific and shared across all providers.
  ...
  query(input: QueryInput): AgentQuery {
    ...
    const sdkQuery = query({
      ...
      options: {
        ...
        env: input.env,
        allowedTools: CLAWIE_TOOL_ALLOWLIST,
...
**Command detection:** Messages starting with `/` are checked against a command list. Recognized commands bypass formatting and are passed raw to the provider (for Claude's slash command handling) or intercepted by the agent-runner (for Clawie-level commands like session reset).
...
The agent-runner runs an MCP server that exposes Clawie tools to the agent. All tools write to the session DB.
...
- **Environment variables:** `AGENT_PROVIDER` (claude/codex/opencode), `CLAWIE_ADMIN_USER_ID`, provider-specific vars (API keys, model overrides), `TZ`
