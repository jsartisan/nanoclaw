...
This document covers **feature skills** — skills that add capabilities via git branch merges. This is the most complex skill type and the primary way Clawie is extended.

Clawie has four types of skills overall. See [CONTRIBUTING.md](../CONTRIBUTING.md) for the full taxonomy:
...
This replaces the previous `skills-engine/` system (three-way file merging, `.clawie/` state, manifest files, replay, backup/restore) with plain git operations and Claude for conflict resolution.

## How It Works

### Repository structure

The upstream repo (`nanocoai/clawie`) maintains:

- `main` — core Clawie (no skill code)
...
- `/setup`, `/debug`, `/update-clawie`, `/customize`, `/update-skills`
...
- Live in the marketplace repo (`nanocoai/clawie-skills`)
...
claude plugin install clawie-skills@clawie-skills --scope project
```
...
Clawie's `.claude/settings.json` registers the official marketplace:
...
```
nanocoai/clawie-skills/
  .claude-plugin/
    marketplace.json              # Plugin catalog
  plugins/
    clawie-skills/              # Single plugin bundling all official skills
      .claude-plugin/
...
Multiple skills are bundled in one plugin — installing `clawie-skills` makes all feature skills available at once. Individual skills don't need separate installation.
...
- Built into `/update-clawie` — after merging main, optionally check for skill updates
...
1. Fork `nanocoai/clawie` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
   git clone https://github.com/<you>/clawie.git
   cd clawie
   ```
3. Run Claude Code:
   ```bash
...
Users who previously ran `git clone https://github.com/nanocoai/clawie.git` and have local customizations:

1. Fork `nanocoai/clawie` on GitHub
2. Reroute remotes:
   ```bash
   git remote rename origin upstream
   git remote add origin https://github.com/<you>/clawie.git
...
3. From this point, `origin` = their fork, `upstream` = nanocoai/clawie
...
In both cases:
- Delete the `.clawie/` directory (no longer needed)
...
This is the same as the existing `/update-clawie` skill's merge path.

### Updating skills

Run `/update-skills` or let `/update-clawie` check after a core update. For each previously-merged skill branch that has new commits, Claude offers to merge the updates.
...
# Create PR from my-fix to nanocoai/clawie:main
```
...
4. Add the skill's SKILL.md to the marketplace repo (`nanocoai/clawie-skills`)
...
1. Maintains a fork of Clawie (e.g., `alice/clawie`)
2. Creates `skill/*` branches on their fork with their custom skills
3. Creates a marketplace repo (e.g., `alice/clawie-skills`) with a `.claude-plugin/marketplace.json` and plugin structure

### Adding a community marketplace

If the community contributor is trusted, they can open a PR to add their marketplace to Clawie's `.claude/settings.json`:
...
Once merged, all Clawie users automatically discover the community marketplace alongside the official one.
...
```bash
claude plugin install alice-skills@alice-clawie-skills --scope project
```
...
- **Community skills use the same merge pattern.** The SKILL.md just points to a different remote:
  ```bash
  git remote add alice https://github.com/alice/clawie.git
  git fetch alice skill/my-cool-feature
  git merge alice/skill/my-cool-feature
  ```
- **Users can also add marketplaces manually.** Even without being listed in settings.json, users can run `/plugin marketplace add alice/clawie-skills` to discover skills from any source.
...
A flavor is a curated fork of Clawie — a combination of skills, custom changes, and configuration tailored for a specific use case (e.g., "Clawie for Sales," "Clawie Minimal," "Clawie for Developers").
...
AskUserQuestion: "Start with a flavor or default Clawie?"
- Default Clawie
- Clawie for Sales — Gmail + Slack + CRM (maintained by alice)
- Clawie Minimal — Telegram-only, lightweight (maintained by bob)

If a flavor is chosen:

```bash
git remote add <flavor-name> https://github.com/alice/clawie.git
...
- `upstream` — `nanocoai/clawie` (core updates)
...
- **GitHub topics** — flavor forks can tag themselves with `clawie-flavor` for searchability
...
- `.clawie/` state directory

Operational skills (`setup`, `debug`, `update-clawie`, `customize`, `update-skills`) remain on main in `.claude/skills/`.
...
Before:
```bash
git clone https://github.com/nanocoai/Clawie.git
cd Clawie
claude
```
...
```
1. Fork nanocoai/clawie on GitHub
2. git clone https://github.com/<you>/clawie.git
3. cd clawie
4. claude
5. /setup
```
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/nanocoai/clawie.git`
- Check if `origin` points to the user's fork (not nanocoai). If it points to nanocoai, guide them through the fork migration.
- **Install marketplace plugin:** `claude plugin install clawie-skills@clawie-skills --scope project` — makes all feature skills available (hot-loaded, no restart)
...
The `.claude/skills/` directory on `main` retains only operational skills (setup, debug, update-clawie, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `claude plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`nanocoai/clawie-skills`) — single Claude Code plugin bundling SKILL.md files for all feature skills
...
>
> We've simplified how skills work in Clawie. Instead of a custom skills engine, skills are now git branches that you merge in.
>
...
> - No more `.clawie/` state directory or skills engine
>
...
>
> **If you previously applied skills via the old system**, your code changes are already in your working tree — nothing to redo. You can delete the `.clawie/` directory. Future skills and updates use the branch-based approach.
>
...
>
> **Want to run your own skill marketplace?** Maintain skill branches on your fork and create a marketplace repo. Open a PR to add it to Clawie's auto-discovered marketplaces — or users can add it manually via `/plugin marketplace add`.
