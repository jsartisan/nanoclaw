/**
 * Detect and clean up unhealthy Clawie peer services.
 *
 * Runs as a setup preflight before we install our own service. A crash-looping
 * peer install (typically the legacy v1 `com.clawie` plist) silently trashes
 * this install's containers on every respawn because its `cleanupOrphans()`
 * reaps anything matching `clawie-`. We scope our reaper by label now, but
...
/**
 * Scan for peer Clawie services and unload any that are crash-looping.
...
function cleanupLaunchdPeers(projectRoot: string): PeerCleanupResult {
  ...
  try {
    plists = fs
      ...
      .filter((f) => /^com\.clawie.*\.plist$/.test(f))
...
function cleanupSystemdPeers(projectRoot: string): PeerCleanupResult {
  ...
  try {
    units = fs
      .readdirSync(unitDir)
      .filter((f) => /^clawie.*\.service$/.test(f))
