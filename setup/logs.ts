 ...
 *     rawLog);
 *
 * clawie.sh emits the bootstrap entry directly via a bash helper so
...
// a CLAWIE_SKIP list when re-executing after a Claude-assisted fix, so
...
/** Wipe prior logs and write a header. Called once per fresh run (by clawie.sh or as a fallback by auto.ts if invoked standalone). */
 ...
 * pre-existing 01-bootstrap.log (written by clawie.sh before this module
