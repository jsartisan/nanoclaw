...
describe('plist generation', () => {
  it('contains the slug-scoped label', () => {
    const projectRoot = '/home/user/clawie';
    ...
    expect(plist).toMatch(/<string>com\.clawie-v2-[0-9a-f]{8}<\/string>/);
  });
  ...
  it('points to dist/index.js', () => {
    ...
    expect(plist).toContain('/home/user/clawie/dist/index.js');
  });
  ...
  it('sets log paths', () => {
    ...
    expect(plist).toContain('clawie.log');
    expect(plist).toContain('clawie.error.log');
  });
});
...
describe('systemd unit generation', () => {
  ...
  it('sets correct ExecStart', () => {
    ...
    expect(unit).toContain(
      'ExecStart=/usr/bin/node /srv/clawie/dist/index.js',
    );
  });
});
...
nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot)}/dist/index.js >> ${JSON.stringify(projectRoot)}/logs/clawie.log 2>> ${JSON.stringify(projectRoot)}/logs/clawie.error.log &
echo $! > ${JSON.stringify(pidFile)}`;

    expect(wrapper).toContain('#!/bin/bash');
    expect(wrapper).toContain('nohup');
    expect(wrapper).toContain(nodePath);
    expect(wrapper).toContain('clawie.pid');
  });
});
