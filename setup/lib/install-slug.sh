...
#   label=$(launchd_label)        # com.clawie-v2-<slug>
#   unit=$(systemd_unit)          # clawie-v2-<slug>
#   image=$(container_image_base) # clawie-agent-v2-<slug>
#
# Slug is sha1(PROJECT_ROOT)[:8] — must match the TS helper exactly so both
# halves of setup name things consistently.

_clawie_install_slug() {
...
launchd_label() {
  printf 'com.clawie-v2-%s' "$(_clawie_install_slug)"
}

systemd_unit() {
  printf 'clawie-v2-%s' "$(_clawie_install_slug)"
}

container_image_base() {
  printf 'clawie-agent-v2-%s' "$(_clawie_install_slug)"
}
