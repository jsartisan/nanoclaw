...
# never fail the caller. Honors CLAWIE_NO_DIAGNOSTICS=1. The distinct_id
...
CLAWIE_PH_KEY='phc_fx1Hhx9ucz8GuaJC8LVZWO8u03yXZZJJ6ObS4yplnaP'
CLAWIE_PH_URL='https://us.i.posthog.com/capture/'
...
ph_install_id() {
  local root="${CLAWIE_PROJECT_ROOT:-${PROJECT_ROOT:-$PWD}}"
...
ph_event() {
  [ "${CLAWIE_NO_DIAGNOSTICS:-}" = "1" ] && return 0
  local event=$1
  shift
  local id
  id=$(ph_install_id)
  ...
  payload=$(printf '{"api_key":"%s","event":"%s","distinct_id":"%s","properties":{%s}}' \
    "$CLAWIE_PH_KEY" "$event" "$id" "$props")

  curl -sS --max-time 3 -X POST "$CLAWIE_PH_URL" \
