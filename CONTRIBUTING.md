...
1. **Check for existing work.** Search open PRs and issues before starting:
   ```bash
   gh pr list --repo nanocoai/clawie --search "<your feature>"
   gh issue list --repo nanocoai/clawie --search "<your feature>"
   ```
...
Clawie uses [Claude Code skills](https://code.claude.com/docs/en/skills) — markdown files with optional supporting files that teach Claude how to do something. There are four types of skills in Clawie, each serving a different purpose.
...
Add capabilities to Clawie by merging a git branch. The SKILL.md contains setup instructions; the actual code lives on a `skill/*` branch.
...
1. Fork `nanocoai/clawie` and branch from `main`
...
**Examples:** `/setup`, `/debug`, `/customize`, `/update-clawie`, `/update-skills`
