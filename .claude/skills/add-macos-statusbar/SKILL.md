...
description: Add a macOS menu bar status indicator for Clawie. Shows a bolt icon with a green/red dot indicating whether Clawie is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows Clawie's running status and lets the user
...
```bash
launchctl list | grep com.clawie.statusbar
```
...
Create `~/Library/LaunchAgents/com.clawie.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.clawie.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.clawie.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see Clawie's status and control the service.
>
> - **Green dot** — Clawie is running
> - **Red dot** — Clawie is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.clawie.statusbar.plist
rm ~/Library/LaunchAgents/com.clawie.statusbar.plist
rm dist/statusbar
```
