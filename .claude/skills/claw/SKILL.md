---
name: claw
description: Install the claw CLI tool — run Clawie agent containers from the command line without opening a chat app.
---

# claw — Clawie CLI

`claw` is a Python CLI that sends prompts directly to a Clawie agent container from the terminal. It reads registered groups from the Clawie database, picks up secrets from `.env`, and pipes a JSON payload into a container run — no chat app required.
...
- Python 3.8 or later
- Clawie installed with a built and tagged container image (`clawie-agent:latest`)
- Either `container` (Apple Container, macOS 15+) or `docker` available in `PATH`

## Install

Run this skill from within the Clawie directory. The script auto-detects its location, so the symlink always points to the right place.
...
You should see registered groups. If Clawie isn't running or the database doesn't exist yet, the list will be empty — that's fine.
...
claw --image clawie-agent:dev "Hello"
...
The script auto-detects your Clawie directory and reads `.env` from it. Check that the file exists and contains at least one of: `CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`.

### Container times out

The default timeout is 300 seconds. For longer tasks, pass `--timeout 600` (or higher). If the container consistently hangs, check that your `clawie-agent:latest` image is up to date by running `./container/build.sh`.
...
### Override the Clawie directory

If `claw` can't find your database or `.env`, set the `CLAWIE_DIR` environment variable:

```bash
export CLAWIE_DIR=/path/to/your/clawie
```
