...
Connect Clawie to Microsoft Teams for interactive chat in team channels, group chats, and direct messages.

## Install

Clawie doesn't ship channels in trunk. This skill copies the Teams adapter in from the `channels` branch.
...
3. Create the Entra app, client secret, and bot registration:
   ...
   The CLI prints the credentials as `CLIENT_ID`, `CLIENT_SECRET`, and `TENANT_ID`. Map them to Clawie's env keys:
...
2. Click **New client secret**, description "clawie", expiry 180 days
...
2. Fill in:
   - **Bot handle**: unique name (e.g., "clawie-bot")
...
```bash
az group create --name clawie-rg --location eastus
az bot create \
  --resource-group clawie-rg \
  --name clawie-bot \
...
```bash
az bot msteams create --resource-group clawie-rg --name clawie-bot
```
...
```json
{
  ...
  "name": { "short": "Clawie", "full": "Clawie Assistant" },
  "description": {
    "short": "Clawie assistant bot",
    "full": "Clawie personal assistant powered by Claude."
  },
...
- **how-to-find-id**: Send a message to the bot in the channel. Clawie auto-creates a messaging group and logs the platform ID. Use that messaging group ID for wiring.
