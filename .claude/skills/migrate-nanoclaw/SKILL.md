...
Clawie users fork the repo and customize it — changing config values, editing source files, modifying personas, adding skills. When upstream ships updates or refactors, `git merge` produces painful conflicts because the same core files were changed on both sides.
...
Check remotes with `git remote -v`. If `upstream` is missing, ask for the URL (default: `https://github.com/nanocoai/clawie.git`), add it, then `git fetch upstream --prune`.
...
Check for existing guide: `.clawie-migrations/guide.md` or `.clawie-migrations/index.md`.

**Determine the tier based on the total diff from base:**

### Tier 1: Lightweight — suggest `/update-clawie` instead
...
Tell the user the scope is small and suggest `/update-clawie` might be simpler. Let them choose.
...
For Tier 1:
- **Use /update-clawie** — simpler merge-based approach
...
> Explore this Clawie fork to identify all changes from the upstream base. Run these commands and report back:
>
...
**Storage:** `.clawie-migrations/guide.md` for Tier 2. `.clawie-migrations/` directory with `index.md` and section files for Tier 3.
...
1. Stop the service (do this directly):
   ```bash
   launchctl unload ~/Library/LaunchAgents/com.clawie.plist 2>/dev/null || true
   ```
...
cp -r "$WORKTREE_PATH/.clawie-migrations" /tmp/clawie-migrations-backup 2>/dev/null || true
...
cp -r /tmp/clawie-migrations-backup/* .clawie-migrations/ 2>/dev/null || true
rm -rf /tmp/clawie-migrations-backup
```
...
1. Use the Read tool to read `.claude/skills/migrate-clawie/diagnostics.md`.
